...
(after! xah-fly-keys
  ...
  ;; (define-key key-translation-map (kbd "ESC") (kbd "C-g")) (add-hook 'doom-escape-hook 'sim/flykeys-indicate-command-mode)
  (global-set-key (kbd "ESC ESC") 'xah-fly-command-mode-activate)

  (message "xah-fly-keys custom settings loaded")
  )

(after! neo-layout-fly-keys
  (neo2/flykeys-on))
#+End_src
...
- =C-F10 = Highlight a Search Pattern with custome color
  #+begin_src emacs-lisp
(general-define-key "S-<f10>" 'highlight-symbol-at-point
...
*** . •
#+begin_src emacs-lisp
(map! :desc "Jumps to next error" "C-•" #'next-error)
#+end_src
*** j J :join:line:
*C-S-j*
...
#+begin_src emacs-lisp
(nk/check-package 'drag-stuff (lambda () (general-define-key
